...
\title{Practical PIR over Permissioned Blockchains: A BGV-based Case Study for CTI Sharing}
...
\begin{abstract}
    The increasing reliance on Cyber Threat Intelligence (CTI) sharing among organizations demands privacy-preserving mechanisms to prevent query leakage while ensuring timely access to threat data. 
    Existing blockchain-based CTI frameworks provide integrity and auditability but reveal access patterns, exposing sensitive organizational interests. 
    In this work, we present the first practical integration of computational Private Information Retrieval (PIR) with a permissioned blockchain framework, focusing on Hyperledger Fabric as a case study. 
    Our system encodes JSON-formatted CTI records into slot-packed ciphertexts using the Brakerski--Gentry--Vaikuntanathan (BGV) homomorphic encryption scheme, enabling PIR queries directly through Fabric chaincode. 
    We introduce a multi-channel architecture---\texttt{channel\_mini}, \texttt{channel\_mid}, and \texttt{channel\_rich}---to flexibly support CTI records of varying sizes. 
    We implement and evaluate our prototype with the Lattigo library, benchmarking encryption latency, chaincode execution time, noise budget evolution, and end-to-end performance across different database scales. 
    Results show that the system achieves practical query latencies and scalability for realistic CTI workloads while preserving query privacy against honest-but-curious peers. 
    This work demonstrates the feasibility of embedding PIR into permissioned blockchains and provides a foundation for future optimizations toward sublinear retrieval cost.
\end{abstract}
    

\begin{IEEEkeywords}
Blockchain, Cyber Threat Intelligence (CTI), Homomorphic Encryption, Hyperledger Fabric, Permissioned Blockchains, Private Information Retrieval (PIR).
\end{IEEEkeywords}
