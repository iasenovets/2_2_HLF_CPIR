...
\noindent\textbf{Implications.} Higher $\log N$ enlarges ring capacity and thus potential $n$, but also forces larger $record_s$ through richer record templates. The discrete allocation policy $\mathcal{S}$ simplifies implementation while bounding overhead. Feasible points emerge only where template requirements align with ring capacity, guiding parameter selection in system deployment.
...
In Hyperledger Fabric, two layers impose constraints:

\textbf{a) World state (LevelDB/CouchDB).} 
Fabric does not set a fixed cap on the number of records $n$; limits are determined by disk capacity and I/O throughput. 
CouchDB enforces a configurable \texttt{max\_document\_size}, which applies to large values such as $m_{\mathrm{DB}}$. 
Our construction stores $m_{\mathrm{DB}}$ as a single world-state entry, plus metadata (\texttt{"n"}, \texttt{"record\_s"}, \texttt{"bgv\_params"}) and optional JSON records (\texttt{"record\%03d"}). 
Feasibility is therefore governed primarily by the size of $m_{\mathrm{DB}}$ and by the slot constraints in Fig.~\ref{fig:exp-matrix}, not by Fabric itself.

\textbf{b) Ledger history (blockchain log).} 
Block size is limited by ordering-service parameters (\texttt{AbsoluteMaxBytes}, \texttt{PreferredMaxBytes}). 
However, PIR queries are executed during the \textsf{evaluate} phase and do not produce blocks. 
Only \textsf{submit} transactions (e.g., \textsf{InitLedger} or record updates) contribute to block payloads. 
Thus block size limits are relevant only when initializing or refreshing $m_{\mathrm{DB}}$, not during PIR queries.

\noindent\textbf{Implication.} 
The effective capacity of a channel is determined by cryptographic feasibility (\S\ref{fig:exp-matrix}) and the maximum size of a single world-state value, rather than by Fabric’s block size. 
For very large CTI objects, a common extension is to store them off-chain (e.g., in IPFS) and persist only their content identifiers (CIDs) in world state, while $m_{\mathrm{DB}}$ continues to support private retrieval of the structured fields.

\subsection{Workflow Narrative}

The complete workflow of our CPIR-enabled Fabric system consists of four stages, illustrated in Fig.~\ref{fig:system-overview}. 
A Data Writer ($\mathcal{DW}$) initializes the ledger by encoding the database $D=\{d_0,\dots,d_{n-1}\}$ into a plaintext polynomial $m_{\mathrm{DB}}$ and submitting it via \textsf{InitLedger}. 
Data Requesters ($\mathcal{DR}$) subsequently discover the necessary parameters by invoking \textsf{GetMetadata}, which exposes only structural information such as $n$, $record_s$, and the BGV parameter set $(\log N,\log Q_i,\log P_i,T)$. 
To privately obtain a record, a $\mathcal{DR}$ constructs a windowed selector ${v}_i$, encrypts it into $ct_q=\mathsf{Enc}_{pk}({v}_i)$, and issues it to the PIR chaincode through \textsf{PIRQuery}. 
Endorsing peers, acting as Data Owners ($\mathcal{DO}$), perform homomorphic evaluation against the stored polynomial to return $ct_r=\mathsf{Eval}(ct_q,m_{\mathrm{DB}})$. 
Finally, the $\mathcal{DR}$ decrypts $ct_r$ using its secret key $sk$ to recover $d_i=\mathsf{Dec}_{sk}(ct_r)$ and reconstructs the original JSON object. 
Throughout this workflow, only opaque ciphertexts and metadata are visible to $\mathcal{DO}$, ensuring that the queried index remains hidden.

\begin{enumerate}
    \item \textbf{DW submits initialization.}  
    $\mathcal{DW}$ calls \textsf{InitLedger} via $\mathcal{GW}$ (\textsf{submit}) with inputs $(n, record_{s,\mathcal{DW}})$ and an optional hint $(\log N,\log Q_i,\log P_i,T)$, where $record_{s,\mathcal{DW}}$ reflects the maximum JSON size anticipated by the writer.

    \item \textbf{DO validates and derives parameters.}  
    $\mathcal{DO}$ computes a provisional slot allocation $record_{s,\mathcal{GW}} = 8 \cdot \lceil record_{s,\mathcal{DW}} / 8 \rceil$.  
    If $\log N$ is absent, the smallest $\log N$ is chosen such that $\mathsf{Cap}(N,record_{s,\mathcal{GW}},n)$ holds.  
    BGV parameters $\{\log N,N,\log Q_i,\log P_i,T\}$ are built and stored.

    \item \textbf{DO prepares records.}  
    Either ingest provided CTI records or synthesize $D=\{d_0,\dots,d_{n-1}\}$ with $|d_i|\le record_{s,\mathcal{DW}}$.

    \item \textbf{Compute slot window.}  
    Fix $record_s = 8 \cdot \lceil \max_i |d_i| / 8 \rceil$ (discrete policy).  

    \item \textbf{Feasibility check.}  
    Reject if any predicate fails:  
    $\mathsf{Min}(\log N,record_s)$, $\mathsf{Disc}(record_s)$, $\mathsf{Cap}(N,record_s,n)$.

    \item \textbf{Pack to coefficient vector.}  
    Assign each $d_i$ to a disjoint slot window $J_i=\{i\cdot record_s,\dots,(i+1)record_s-1\}$ in $c=(c_0,\dots,c_{N-1})$; pad with zeros.

    \item \textbf{Encode polynomial and persist.}  
    Encode $m_{\mathrm{DB}}(X)=\sum_{j=0}^{N-1} c_j X^j \in \mathbb{Z}_T[X]/(X^N+1)$ at max level.  
    Persist to world state under keys \texttt{"m\_DB"}, \texttt{"n"}, \texttt{"record\_s"}, and \texttt{"bgv\_params"} $=\{\log N,N,\log Q_i,\log P_i,T\}$, plus optional \texttt{"record\%03d"} entries.

    \item \textbf{DR discovers metadata.}  
    $\mathcal{DR}$ invokes \textsf{GetMetadata} (\textsf{evaluate}).  
    $\mathcal{DO}$ returns $(n,record_s,\log N,N,T,\log Q_i,\log P_i)$.

    \item \textbf{DR instantiates crypto context.}  
    From metadata, $\mathcal{DR}$ builds parameters, runs $\mathsf{KeyGen}(\lambda)\!\to\!(pk,sk)$, and prepares encoder/encryptor.

    \item \textbf{Form the windowed selector.}  
    For index $i\in[n]$, define the window $J_i=\{i\cdot record_s,\dots,(i+1)\cdot record_s-1\}$.  
    Build ${v}_i\in\{0,1\}^{N}$ with ones on $J_i$ and zeros elsewhere.

    \item \textbf{Encode and encrypt the query.}  
    Encode ${v}_i$ as $m_q(X)=\sum_{j=0}^{N-1} ({v}_i)_j X^j$ and encrypt $ct_q=\mathsf{Enc}_{pk}({v}_i)$.  
    Serialize and Base64-encode the ciphertext.

    \item \textbf{DR issues PIR query.}  
    $\mathcal{DR}$ calls \textsf{PIRQuery}(\texttt{encQueryB64}) via $\mathcal{GW}$ (\textsf{evaluate}).

    \item \textbf{DO evaluates homomorphically.}  
    $\mathcal{DO}$ decodes $ct_q$, ensures $m_{\mathrm{DB}}$ is loaded from world state, and computes $ct_r=\mathsf{Eval}(ct_q,m_{\mathrm{DB}})$ (ciphertext–plaintext multiply).  
    The result is serialized and Base64-encoded for return.

    \item \textbf{DR decrypts the response.}  
    $\mathcal{DR}$ decodes and decrypts $ct_r$ to $m'(X)$, decodes slots, and extracts bytes on $J_i$.

    \item \textbf{Reconstruction and validation.}  
    Trim at padding zero, decode UTF-8, validate JSON, and output $d_i$.  
    If $record_s=1$, return the scalar value directly.
\end{enumerate}
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
    \REQUIRE $n \in \mathbb{N}$; $record_s^{\mathcal{DW}} \in \{0;1\}^{\ast}$; op: $\mathrm{hint}$
