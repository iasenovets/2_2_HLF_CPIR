...
\title{HLFCPIR: Embedding Computational Private Information Retrieval into Hyperledger Fabric Chaincode for Query Privacy}
...
\begin{abstract}
    ...
    In Hyperledger Fabric, \textsf{evaluate} transactions reveal function arguments and read-sets to peers, creating privacy risks for organizations querying sensitive records. 
    We address this gap by presenting the first full integration of Computational Private Information Retrieval (CPIR) within Fabric’s chaincode layer. 
    Our design encodes the ledger’s key–value table as a packed plaintext polynomial and enables clients to submit encrypted selection vectors evaluated under the Brakerski–Gentry–Vaikuntanathan (BGV) homomorphic encryption scheme. 
    Peers perform ciphertext–plaintext evaluation entirely inside chaincode and return encrypted results, preventing index leakage while preserving Fabric’s endorsement and audit semantics. 
    A prototype built with the Lattigo library achieves end-to-end query latency of $\approx$113~ms and peer-side evaluation below~45~ms for databases of up to~128~records, demonstrating practical performance within permissioned environments. 
    Network measurements indicate $\approx$2~MB of peer traffic per query due to the external chaincode model, which can be halved under in-process deployment. 
    Overall, the results confirm the feasibility of native CPIR execution in Hyperledger Fabric and establish a foundation for scalable extensions such as constant-time evaluation, sublinear and sharded PIR, and encrypted-database (\emph{ct×ct}) computation for richer private analytics.
\end{abstract}
...
Blockchain \cite{nakamoto_bitcoin_nodate} is a peer-to-peer network protocols that uses cryptographic primitives and 
consensus mechanism to create and maintain a distributed ledger of transactions, such as Ethereum \cite{wood_ethereum_nodate} and 
Hyperledger Fabric \cite{androulaki_hyperledger_2018}.

According to the literature \cite{dunnett_trusted_2022, allouche_trade_2021, gong_blocis_2020, huff_distributed_2021} 
such systems are wideply adopted in various domains, such as Cyber Threat Intelligence (CTI) sharing, 
where multiple organizations exchange threat indicators and wish to keep their queries confidential.
Blockchain guarantees, however, primarily cover data integrity and auditability, not query privacy. For example, only recently
PSE team in Ethereum foundation explicitly highlighted the need for private reads in addition to private writes and 
private proving~\cite{ethereum_pse_nodate}.

In Hyperledger Fabric \cite{androulaki_hyperledger_2018}, the separation of \emph{evaluate} and \emph{submit} 
makes the read-privacy gap explicit: an evaluate call is a read-only proposal sent to endorsing peers, 
which execute the chaincode and return results without committing to the ledger \cite{noauthor_fabric_gateway_nodate}.
Crucially, these peers still observe all function arguments and read-sets. 
Thus, in multi-organization settings, the dominant privacy risk arises not from the immutable ledger, 
but from endorsing peers who can log or infer sensitive query information.

This motivates us to target \emph{query} or \emph{read} privacy issue in Fabric, defined as the ability to 
...
There are two main types of PIR schemes.
Earlier Information-Theoretic (IT-PIR) schemes~\cite{chor_private_1998,demmler_raid-pir_2014,goldberg_improving_2007,
beimel_breaking_2002, devet_optimally_2012} provide unconditional privacy by distributing the database across multiple 
non-colluding servers, a client then queries subsets of servers such that no single server learns the selection index. 
Computational PIR (CPIR) schemes~\cite{cachin_computationally_1999, chang_single_2004, 
gentry_single-database_2005, brakerski_efficient_2011, dong_fast_2014, aguilar-melchor_xpir_2016} relying on cryptographic assumptions 
and homomorphic encryption to achieve privacy with a single server. 

However, consortium settings exclude IT-PIR due to multiple non-colluding servers requirement, since all peers are typically considered 
honest-but-curious, and collusion is a realistic threat. Thus, we focus on CPIR, which is more suitable for our threat model.

\subsection{LITERATURE REVIEW}
This section surveys prior work at the intersection of Private Information Retrieval (PIR) and distributed systems, 
with emphasis on smart-contract or chaincode-based implementations for query privacy across different domains, 
not limited to any specific domain or blockchain platform.

Xie et al.~\cite{xie_private_2023} design BIT-PIR, a PIR-based query scheme tailored to Bitcoin light clients: they restructure 
on-chain data and craft a storage format so lightweight wallets can privately fetch transaction-related items without Bloom-filter leakage. 
This work is pioneering in applying PIR to public blockchains, but its reliance on Bitcoin's UTXO model limits generalizability to other platforms. 

Chhabra et al. and Kumar introduced a series of PIR - blockchain frameworks across multiple domains, such as
PRIBANI, MUDRA, DEBPIR, and BRON. PRIBANI~\cite{chhabra_pribani_2024} combines PIR with ring signatures and PEKS to 
enhance query anonymity in transaction systems. MUDRA~\cite{chhabra_mudra_2025} integrates dual-server Ring-PIR within Hyperledger Fabric 
to enable private access to healthcare records, while DEBPIR~\cite{kumar_debpir_2025} embeds PIR and k-out-of-N OT 
in Fabric chaincode for privacy-preserving business data retrieval. BRON~\cite{kumar_bron_2024} extends this approach 
to human resource management, integrating PIR at the smart-contract layer with ZK components to support verifiable private record queries. 
Across these works, PIR consistently serves as a privacy-preserving layer for permissioned ledgers, 
yielding favorable throughput and latency. However, their evaluations offer limited cryptographic parameter details, making scalability 
and precise overhead less transparent.

Xiao et al.~\cite{xiao_cloak_2024} propose Cloak, a privacy-preserving blockchain query framework that hides retrieval targets via two-phase 
distributed query requests (division and aggregation). By splitting queries across nodes and aggregating partial answers, 
Cloak reduces exposure of which records are fetched and reports notable query-time improvements over naive single-server approaches. 
The technique is PIR-inspired rather than a direct PIR scheme; the paper focuses on distributed obfuscation and leaves a comparison 
against concrete CPIR/IT-PIR baselines (communication vs. server work vs. anonymity) for future study.

Peer2PIR~\cite{mazmudar_peer2pir_2024} addresses privacy leakage in IPFS by enabling private peer routing, provider discovery, 
and content retrieval using PIR-backed protocols; the paper also surveys PIR choices in distributed systems and 
demonstrates reasonable overheads. While not a blockchain system per se, the work is highly relevant to decentralized 
Web stacks frequently coupled with blockchains, and its modular designs could be adapted to ledger-attached DHTs. 
A limitation is the absence of end-to-end integration with on-chain settlement/audit, so operational lessons for smart-contract 
pipelines are indirect.

\textbf{Targeted gap.}
Given the above survey, we identify a gap in the literature: while several works embed PIR into permissioned blockchains, 
none provide a detailed, practical implementation of a lattice-based CPIR scheme directly within Hyperledger Fabric chaincode,
with comprehensive parameter analysis and performance benchmarks.
One recent survey outlined several open research questions in this area~\cite{chhabra_navigating_2024}, such as: How can PIR be effectively 
integrated into blockchain to enhance privacy in decentralized applications?

Our work directly contributes to these directions by implementing a HE-based CPIR protocol in Fabric chaincode under evaluate calls,
ensuring that endorsing peers cannot infer queried indices while also answering the open research question above.

Combining Fabric with CPIR thus yields a dual guarantee: (1) writes remain auditable and attributable; 
(2) reads become unlinkable to specific keys—even to endorsing peers.

\subsection{KEY OBJECTIVE AND CONTRIBUTIONS}
In this work, we demonstrate that CPIR can be implemented natively in chaincode to provide query privacy for Fabric reads under evaluate path
and achieve practical performance under certain parametrization choices.

Hence, the main contributions of this work are:
\begin{enumerate}
    \item \textbf{BGV-based CPIR as Fabric chaincode.} 
    We present, to the best of our knowledge, the first fully functional integration 
    of a lattice-based CPIR scheme into Fabric chaincode for query privacy.
    \item \textbf{Feasibility constraints.} 
    We derive necessary and sufficient conditions on the cryptographic parameters, database size, 
    and record structure to ensure that polynomial packing is possible, facilitating practical deployment for CTI sharing and similar use cases.
    \item \textbf{Multi-channel architecture.} 
    We design a multi-channel Fabric architecture where each channel maintains an 
    independent polynomial database and metadata, allowing different organizations to host separate datasets while reusing the same CPIR chaincode.
    \item \textbf{Comprehensive evaluation.} 
    We implement a prototype using the Lattigo library and benchmark cryptographic operations, 
    ...
    \item \textbf{Open-Source Release.} To encourage reproducibility and use by other researchers, we open-source 
    the complete CPIR-on-Blockchain system, including chaincode, client, and experimental setup. 
    The repository is available at: \url{https://github.com/artias13/2_2_HLF_CPIR}.
\end{enumerate}

\subsection{ORGANIZATION}

The remainder of this paper is organized as follows: 
Section~\ref{sec:preliminaries} reviews Fabric privacy features, PIR fundamentals, and BGV encryption.
Section~\ref{sec:proposed_system} describes our system and threat models, polynomial database construction, feasibility constraints, multi-channel architecture, workflow and algorithms.
...
\textbf{Separate Channels.} Fabric's multi-channel architecture~\cite{noauthor_channels_nodate} isolates ledgers across subgroups of organizations, limiting which participants observe which data. 
However, channel separation controls \emph{who} sees a ledger, query intent remains visible to all endorsers of a channel.

\textbf{Private Data Collections (PDC).} PDCs~\cite{noauthor_private_nodate} restrict which organizations store and access private key–value pairs. 
...
\textbf{Fabric Private Chaincode (FPC).} FPC~\cite{noauthor_hyperledgerfabric-private-chaincode_nodate,brandenburger_blockchain_2018} executes chaincode within Intel SGX enclaves. 
...
LevelDB is sufficient and preferable for performance and simplicity, while CouchDB~\cite{noauthor_couchdb_nodate} remains an option to extend from 8~MB to 4~GB if needed.

\textbf{Ledger history (blockchain log).}
According to the documentation~\cite{noauthor_performance_nodate}, block size is constrained by the ordering service configuration. 
...
In our system, these limits affect only \emph{submit} transactions such as \emph{InitLedger} or record updates. 
\emph{Evaluate} transactions (including \emph{PIRQuery}) are read-only and do not generate blocks, thus unaffected by ordering or batching constraints.
...
For large or binary CTI objects (e.g., malware samples or full PCAPs), an optional extension is to store them off-chain 
...
(BGV), CKKS and TFHE~\cite{fan_somewhat_2012,brakerski_fully_2011,cheon_homomorphic_2017,chillotti_tfhe_2018} have emerged as 
...
He then encrypts it as $ct_q = \text{Enc}_{pk}(\hat{v}_i)$ under a public key $pk$ and sends $ct_q$ to the server, which computes 
$ct_r = (ct_q \cdot D) = \text{Enc}_{pk}(d_i)$, returning $ct_r$ to the client for decryption $d_i = \text{Dec}_{sk}(ct_r)$ 
...
\begin{itemize}
    ...
    \item $\emph{Enc}_{pk}(\hat{v}_i) \rightarrow ct_q$: Given a windowed selection vector $\hat{v}_i \in \{0,1\}^N$, encode it into the plaintext ring $R_T$ and encrypt to a query ciphertext $ct_q$ under $pk$.
    \item $\emph{Eval}(ct_q, \emph{$m_{DB}$}) \rightarrow ct_r$: Given $ct_q$ and the plaintext polynomial database $\emph{$m_{DB}$} \in R_T$, homomorphically evaluate the product to obtain an encrypted response $ct_r \in R_Q$.
...
since the database polynomial \emph{$m_{DB}$} is kept in plaintext within world state. 
...
(relinearization keys, Galois keys, encrypted \emph{$m_{DB}$}), larger world-state footprint, and higher evaluation cost, 
...
\renewcommand{\arraystretch}{1.1}
    ...
    \hline
    $ct_q=\emph{Enc}_{pk}(\hat{v}_i)$ & Encrypted query \\
    \hline
    $ct_r=\emph{Eval}(ct_q, \emph{$m_{DB}$})$ & Encrypted response \\
    \hline
    $d_i=\emph{Dec}_{sk}(ct_r)$ & Decrypted record $d_i$ retrieved by client \\
    \hline
    $\emph{KeyGen}(\lambda)\!\to\!(pk,sk)$ & Key generation \\
    \hline
    ...
    \hline
    ${c}=(c_0,\dots,c_{N-1})$ & Coefficient vector of \\ & the polynomial encoding (slots of \emph{$m_{DB}$}) \\
    \hline
    ...
    \hline
    $\mathsf{Cap} \wedge \mathsf{Min} \wedge \mathsf{Disc}$ & Feasibility condition \\
    \hline
    $\mathcal{DO},\,\mathcal{DW},\,\mathcal{DR},\,\mathcal{GW}$ & Data Owner; Data Writer; \\ & Data Requester; Gateway \\
    \hline
    \emph{evaluate}, \emph{submit} & Fabric read / write transaction phases \\
    \hline
...
The novelty of our approach lies in the integration of CPIR into Hyperledger Fabric chaincode using the BGV~\cite{brakerski_fully_2011} homomorphic encryption scheme. 
...
\begin{itemize}
    \item \textbf{Data Owner ($\mathcal{DO}$):} Endorsing peers that hold the current plaintext polynomial \emph{$m_{DB}$} in world state and execute PIR during \emph{evaluate}. $\mathcal{DO}$ is honest-but-curious.
    \item \textbf{Data Writer ($\mathcal{DW}$):} A client organization that provisions or refreshes the database. $\mathcal{DW}$ invokes \emph{submit} to initialize the ledger (e.g., set $n$ and template bounds). Chaincode computes $record_s$, packs $D=\{d_0,\dots,d_{n-1}\}$, encodes it into \emph{$m_{DB}$}, and persists it.
    \item \textbf{Data Requester ($\mathcal{DR}$):} A client that privately retrieves a record. $\mathcal{DR}$ runs $\emph{KeyGen}(\lambda)\!\to\!(pk,sk)$, forms $ct_q=\emph{Enc}_{pk}(v_i)$, calls \emph{evaluate} \emph{PIRQuery}, and later decrypts $ct_r$.
    \item \textbf{Gateway ($\mathcal{GW}$):} The Fabric client/chaincode interface used by $\mathcal{DW}$ and $\mathcal{DR}$ to invoke \emph{InitLedger}, \emph{GetMetadata}, and \emph{PIRQuery}. It follows standard Fabric semantics; no extra trust is assumed.
\end{itemize}

\noindent\textit{Remark (world-state scope).} In Fabric, the "ledger" comprises the blockchain log and the world state. Our CPIR operates on the world state: \emph{$m_{DB}$} encodes the latest key--value snapshot, not the historical transaction logs.
...
\begin{itemize}
    \item \textbf{Endorsing peers ($\mathcal{DO}$).} Execute chaincode correctly but may try to infer the queried index from \emph{evaluate} inputs or logs. They see $ct_q$, metadata, and \emph{$m_{DB}$}.
    \item \textbf{Data Writer ($\mathcal{DW}$).} Issues initialization writes via \emph{submit}. $\mathcal{DW}$ is not trusted with decryption keys and learns nothing about $\mathcal{DR}$’s queries. We assume $\mathcal{DW}$ follows the write protocol but is not relied upon for privacy.
...
\begin{enumerate}
    ...
    $\mathcal{DW}$ invokes \emph{InitLedger} via $\mathcal{GW}$ using \emph{submit}. Chaincode derives $record_s$ from $record_b$, packs $D$ into ${c}=(c_0,\dots,c_{N-1})$, encodes \emph{$m_{DB}$}, and stores \emph{$m_{DB}$} and metadata in world state held by $\mathcal{DO}$.
    \item \textbf{Metadata discovery.}
    $\mathcal{DR}$ calls \emph{GetMetadata} via \emph{evaluate} to obtain $n$, $record_s$, and BGV parameters needed to form a valid query.
    \item \textbf{Private retrieval.}
    $\mathcal{DR}$ constructs $ct_q=\emph{Enc}_{pk}(v_i)$ and invokes \emph{PIRQuery} via \emph{evaluate}. $\mathcal{DO}$ computes $ct_r=\emph{Eval}(ct_q,\emph{$m_{DB}$})$ and returns it.
...
\begin{figure}[!t]
    ...
    \caption{“Workflow. $\mathcal{DW}$ initializes the ledger via $\mathcal{GW}$, which triggers chaincode on endorsing peers ($\mathcal{DO}$). $\mathcal{DO}$ executes the protocol and persists state in world state (\emph{$m_{DB}$}, metadata, JSON records). $\mathcal{DR}$ later obtains metadata, submits $ct_q=\emph{Enc}{pk}(v_i)$, $\mathcal{DO}$ evaluates $ct_r=\emph{Eval}(ct_q,m{\mathrm{DB}})$ against world state, and $\mathcal{DR}$ decrypts to $d_i$.}
...
Endorsing peers operate over \emph{$m_{DB}$} during PIR queries, while clients recover only the slots corresponding to their requested record.
...
\begin{itemize}
    ...
    \item The \emph{normal view}: JSON records stored individually under keys \emph{"record\%03d"} for auditability and interoperability with non-PIR chaincode.
    \item \emph{Metadata}: 
    \begin{itemize}
...
A practical concern is the maximum size of the ledger when channels store both JSON records and the polynomial \emph{$m_{DB}$}. 
...
refer to Section~\ref{sec:preliminaries} for details.
...
(\emph{InitLedger}, \emph{GetMetadata}, \emph{PIRQuery}) and the client-side routines 
(\emph{FormSelectionVector}, \emph{DecryptResult}). 
...
\begin{enumerate}
    ...
    $\mathcal{DW}$ calls \emph{InitLedger} (Alg.~\ref{alg:initledger}) via $\mathcal{GW}$ (\emph{submit}) 
    ...
    $\mathcal{DR}$ calls \emph{GetMetadata} (Alg.~\ref{alg:getmetadata}) via \emph{evaluate} 
    ...
    For index $i\in[n]$, $\mathcal{DR}$ runs \emph{FormSelectionVector} (Alg.~\ref{alg:encryptquery}): 
    ...
    $\mathcal{DR}$ issues \emph{PIRQuery}($ct_q^{B64}$) (Alg.~\ref{alg:pirquery}) via \emph{evaluate}. 
    ...
    $\mathcal{DR}$ runs \emph{DecryptResult} (Alg.~\ref{alg:decryptresult}) to recover $m'(X)$, 
...
If \emph{$m_{DB}$} is re-encoded or refreshed, $\mathcal{GW}$ still returns the same metadata blob; clients rebuild context idempotently.
...
Our implementation employs the \emph{Lattigo}~v6 library~\cite{noauthor_lattigo_2024} as the homomorphic encryption backend. 
Lattigo provides a Go-native implementation of the BGV scheme~\cite{brakerski_fully_2011},
...
Client–peer communication was performed through the Fabric~Go~SDK~\cite{noauthor_client_nodate}. 
...
\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/plots/plate_col.pdf}
...
average \textit{server-side execution time} of the main chaincode functions across different ring sizes.
...
Transactions that modify world state (e.g., \emph{InitLedger}) are issued via the 
\emph{submit} path and committed through Raft consensus, 
while read-only operations (\emph{GetMetadata}, \emph{PIRQuery}) use the 
\emph{evaluate} path, bypassing block creation and ordering.  
All metrics are averaged over 20~executions under identical peer configurations.

\subsection{OVERALL SYSTEM PERFORMANCE}

\begin{figure*}[!ht]
    \centering
    \includegraphics[width=\textwidth]{figures/diagrams/5_query_privacy.png}
...
(\emph{KeyGen}, \emph{Enc}, \emph{Eval}, and \emph{Dec}) for each evaluated ring dimension~$\log N$. 
...
On-chain network behavior is reported in Table~\ref{tab:pirquery-netio}, 
which presents the average peer-side network I/O per \emph{PIRQuery} transaction 
across the three channel configurations. 
Measured chaincode execution times are provided in Table~\ref{tab:chaincode-timings},
for \emph{InitLedger}, \emph{GetMetadata}, and \emph{PIRQuery} functions 
under identical peer conditions. 
Additionally, Figure \ref{fig:query-privacy} illustrates the peer- and client-side execution during a PIR query
...
\begin{table}[!h]
    ...
    \hline
    $\log N$ & pk & sk & $ct_q$ & $ct_r$ & \emph{$m_{DB}$} & Metadata \\
    \hline
...
The first workflow corresponds to $\mathcal{DW}$ initializing the ledger through \emph{InitLedger}, 
...
executing \emph{GetMetadata} → \emph{KeyGen} → \emph{Enc} → \emph{PIRQuery} → \emph{Dec}.
...
\begin{table}[!h]
    ...
    \hline
    \textbf{Workflow} & 
    \shortstack{\textbf{Cryptographic} \\ \textbf{Operations (ms)}} & 
    \shortstack{\textbf{Blockchain} \\ \textbf{Operations (ms)}} & 
    \shortstack{\textbf{Total Time} \\ \textbf{(ms)}} \\
    \hline
...
While the \emph{InitLedger} operation incurs higher cost due to state updates and ordering, 
the end-to-end query workflow completes within $\approx$113~ms on average.

\textbf{Batched Queries.}
Among our results, for the ring size $\log N{=}15$ configuration, issuing 100/1000/10000 private queries incurs 
...
\begin{table}[!h]
    \centering
    \caption{Projected cost for batched PIR queries at $\log N{=}15$ (peer perspective)}
...
\begin{figure}[!h] 
    \centering 
    \includegraphics[width=\columnwidth]{figures/plots/pirquery_batch_cost_bw.pdf} 
    \caption{Projected cost for batched PIR queries at $\log N{=}15$ (peer perspective)} 
    \label{fig:batch-cost-rich} 
\end{figure}

\noindent\textit{Remark (Why 2x to just $ct_q$ + $ct_r$?).} 
The network I/O values in Table~\ref{tab:pirquery-netio} and Table~\ref{tab:batch-cost-rich} 
were obtained from \emph{docker stats} snapshots collected for the 
\emph{peer0.org1.example.com} and \emph{orderer0.group1.orderer.example.com} 
containers during each benchmark epoch. 
For every execution of \emph{PIRQuery}, the benchmarking script recorded 
cumulative network statistics (\emph{NET~I/O}) before and after the transaction, 
and the Python post-processing pipeline computed per-epoch deltas 
(\emph{bytes\textsubscript{in/out}(t)}~--~\emph{bytes\textsubscript{in/out}(t--1)}) 
to obtain the mean transfer volume per function. 
Because the chaincode was deployed as an \emph{external service container}, 
each PIR transaction traversed two gRPC hops: 
(1)~client~$\leftrightarrow$~peer and 
(2)~peer~$\leftrightarrow$~chaincode. 
Each ciphertext---the query $ct_q$ and the response $ct_r$---flows in both directions across these links, 
resulting in roughly four full transmissions per query. 
This accounts for the observed aggregate of $\approx$2072~KB per transaction 
for $\log N{=}15$, which aligns with the theoretical estimate 
$4\times512\text{~KB}\,{=}\,2048\text{~KB}$ plus gRPC/TLS framing overhead. 
If the same chaincode were executed \emph{in-process} (within the peer container), 
only the client--peer exchange would occur, reducing the network footprint 
to approximately $2\times512{=}$1024~KB per query or 10 GB for 10,000 queries.

\section{DISCUSSION}\label{sec:discussion}

\subsection{RELATED WORK COMPARISON}

Table~\ref{tab:comparison-related} contrasts representative blockchain privacy frameworks 
...
The comparison focuses on five key aspects:
\emph{(i)} the underlying privacy primitive (CPIR, IT-PIR, or hybrid),
\emph{(ii)} whether privacy computation occurs on-chain (via smart contracts or chaincode) or off-chain,
\emph{(iii)} the transaction phase in which privacy logic executes (\textsf{evaluate} vs. \textsf{submit}),
\emph{(iv)} typical database or world-state size, and
\emph{(v)} reported end-to-end (E2E) query latency when available.

\noindent
\textbf{Analysis.}
Prior blockchain–PIR integrations vary widely in scope and performance.
Permissionless systems such as \textit{Cloak} or \textit{Peer2PIR} rely on 
distributed IT-PIR layers that incur multi-second latencies due to cross-node communication.
Permissioned frameworks such as \textit{PRIBANI}, \textit{MUDRA}, and \textit{BRON}
embed PIR coordination logic in smart contracts or chaincode,
but delegate the actual cryptographic computation to off-chain modules.
Their reported end-to-end query times range between $400$ and $900$~ms for databases of a few hundred records.

In contrast, our \textit{BGV-based CPIR-on-Fabric} performs the PIR evaluation entirely
inside the Fabric chaincode during the \textsf{evaluate} phase,
achieving an average query latency of approximately $113$~ms for world states of $70$–$130$ records 
(256~KB total size). This includes both cryptographic and blockchain processing.
To our knowledge, this is the first implementation to realize a 
fully functional BGV-based CPIR entirely within Hyperledger Fabric chaincode, 
achieving sub-200~ms end-to-end under evaluate invocations in the query workflow.

\begin{table*}[!h]
    \centering
    \caption{Comparison with existing blockchain privacy frameworks}
    \label{tab:comparison-related}
        \begin{tabular}{|c|c|c|c|c|c|c|}
        \hline
        \textbf{Work} &  
        \shortstack{\textbf{Permissioned} / \\ \textbf{Permissionless}} &
        \shortstack{\textbf{Privacy} / \\ \textbf{Technique}} &
        \shortstack{\textbf{Chaincode} / \\ \textbf{Off-chain}} &
        \shortstack{\textbf{Evaluate} / \\ \textbf{Submit Phase}} &
        \shortstack{\textbf{World State }/ \\ \textbf{DB Size}} &
        \shortstack{\textbf{E2E Query} / \\ \textbf{Time (ms)}} \\
        \hline
        \cite{xie_private_2023} & - & IT-PIR + ZKP & Off-chain service & Off-chain & - & - \\
        \hline
        \cite{chhabra_pribani_2024} & Permissioned  & CPIR & Chaincode orchestration & Evaluate & - & - \\
        \hline
        \cite{chhabra_mudra_2025} & Permissioned  & CPIR + Differential Privacy & Chaincode + off-chain & Evaluate & - & - \\
        \hline
        \cite{kumar_debpir_2025} & Permissioned  & IT-PIR (OT-based) & Chaincode + off-chain peers & Evaluate & - & - \\
        \hline
        \cite{kumar_bron_2024} & Permissioned  & CPIR + ZKP & Smart contract & Submit & - & - \\
        \hline
        \cite{xiao_cloak_2024} & -  & Distributed IT-PIR & Hybrid contracts & Submit & - & - \\
        \hline
        \cite{mazmudar_peer2pir_2024} & -  & IT-PIR + P2P privacy & Off-chain IPFS layer & Off-chain & - & - \\
        \textbf{Ours} & Permissioned  & \textbf{CPIR (lattice-based, BGV)} & \textbf{Chaincode} & \textbf{Evaluate} & \textbf{Yes} & \textbf{Yes} \\
        \hline
        \end{tabular}
\end{table*}
...
While the proposed system demonstrates the feasibility of integrating CPIR directly 
into Hyperledger Fabric’s chaincode layer for privacy-preserving queries, 
several aspects remain open for further optimization and scalability improvements.

\textbf{Leakage and timing resilience.}
Although the query index remains cryptographically hidden from endorsing peers, 
side-channel indicators such as ciphertext size and response latency may still 
reveal coarse information about dataset scale or structure. 
Future work should explore constant-time chaincode execution and standardized ciphertext 
serialization to minimize observable timing and size variability.

\textbf{Sublinear and sharded execution.}
The current CPIR design maintains linear complexity in database size $n$, 
as each query involves homomorphic multiplication of the selection vector with the 
entire packed plaintext database $m_{\mathrm{DB}}$. 
Scaling to larger consortium datasets will require integrating sharding or 
sublinear approaches—such as preprocessing-based PIR, multi-stage hinting, 
or offline initialization phases—to reduce online evaluation cost.

\textbf{Encrypted-database mode.}
Extending the ciphertext–plaintext (\emph{ct}×\emph{pt}) evaluation 
to a fully encrypted-database (\emph{ct}×\emph{ct}) mode would 
enable richer on-chain analytics under encryption. 
This transition would necessitate the use of relinearization and rotation keys, 
modulus-switching support, and careful parameter calibration to 
preserve correctness and noise margins within Fabric’s resource limits.

\textbf{Dynamic updates and record addition.}
Introducing an \textsf{addRecord} operation into the workflow 
would allow incremental ledger updates without reinitializing 
the entire $m_{\mathrm{DB}}$. 
This raises new design questions around database re-encoding, key refresh, 
and homomorphic consistency across epochs.

\textbf{Selective field-level retrieval.}
Adaptive packing strategies could allocate slot windows dynamically 
based on JSON field relevance, enabling field-level access control 
and partial record retrieval while maintaining algebraic correctness.

\textbf{Deployment considerations.}
From a system engineering perspective, integrating homomorphic encryption 
within Fabric peers introduces bandwidth and serialization overheads. 
In our setup, the external-chaincode deployment model caused 
each query and response ciphertext (\emph{ct\textsubscript{q}}, \emph{ct\textsubscript{r}}) 
to traverse two gRPC hops, effectively doubling network traffic per ciphertext. 
Future implementations should investigate in-process or sidecar-based 
execution models to reduce redundant communication and 
improve overall throughput.

\textbf{Dual-mode CPIR operation.}
Finally, extending the architecture to support dual-mode queries—covering both 
the current world state and historical ledger data—could enable 
privacy-preserving analytics and provenance tracking. 
Such a capability would align CPIR with emerging research on 
private reads and verifiable computation in distributed ledgers~\cite{chhabra_navigating_2024}.

\section{CONCLUSION}\label{sec:conclusion}
This work demonstrates that privacy-preserving query execution through computational private information retrieval (CPIR) 
can be realized natively inside Hyperledger~Fabric’s chaincode layer. 
By embedding a lattice-based BGV scheme directly within Fabric’s \textsf{evaluate} transaction path, 
we show that it is possible to preserve query privacy without modifying the underlying blockchain consensus or storage layers.

Our implementation achieves end-to-end query completion in $\approx$113~ms on average 
for world states of 70–130 records (256~KB total), 
and chaincode-level evaluation times below 45~ms even at $\log N{=}15$. 
Cryptographic operations remain lightweight—key generation, encryption, and homomorphic evaluation 
complete within 55~ms, 50~ms, and 65~ms respectively—while maintaining deterministic correctness 
under the BGV parameter sets validated in Table~\ref{tab:params}. 
Although ciphertexts grow linearly with the ring dimension (512~KB at $\log N{=}15$), 
peer CPU and memory utilization remained modest throughout the experiments. 

Network behavior, captured through \texttt{docker stats} instrumentation, 
revealed a mean peer-side I/O of $\approx$2.0~MB per private query, 
explained by the dual-hop communication pattern of the external chaincode architecture. 
At this rate, 100, 1000, and 10{,}000 queries correspond to approximately 
207~MB, 2.07~GB, and 20.7~GB of total peer network traffic, respectively. 
While such throughput requirements may challenge large-scale deployments, 
they remain acceptable in controlled consortium settings and can be 
significantly reduced by adopting in-process or sidecar-based execution models.

Overall, these results confirm the practicality of implementing CPIR 
directly within a permissioned blockchain framework for privacy-preserving reads, 
establishing a foundation for future research on scalable, efficient, and composable 
private-query mechanisms over distributed ledgers. 
Key directions for advancement include: (i)~constant-time chaincode evaluation and 
standardized ciphertext serialization to mitigate side-channel leakage; 
(ii)~sublinear or sharded CPIR constructions for scalability; 
(iii)~encrypted-database (\emph{ct×ct}) evaluation for richer analytics; 
and (iv)~deployment optimizations to minimize redundant communication 
in peer–chaincode interactions. 

In conclusion, our BGV-based CPIR-on-Fabric prototype illustrates that 
cryptographically private queries can coexist with blockchain endorsement semantics, 
achieving millisecond-scale latency for single queries and maintaining 
chaincode modularity and auditability—paving the way toward 
natively private data access in permissioned ledgers.
