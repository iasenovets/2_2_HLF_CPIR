...
\begin{table}[!t]
    ...
    \renewcommand{\arraystretch}{1.1}
        ...
        \hline
        $ct_q=\mathsf{Enc}_{pk}(\mathbf{v}_i)$ & Encrypted query \\
        \hline
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{"bgv\_params"}: cryptographic parameters $\{\log N, N, \log Q_i, \log P_i, T\}$.
    \end{itemize}
\end{itemize}
This dual representation ensures that endorsing peers can serve conventional read requests while PIR queries operate exclusively on the polynomial $m_{\mathrm{DB}}$. 
Metadata allows clients to reconstruct the cryptographic context (parameters, slot policy, capacity) needed to form valid queries.
...
\begin{itemize}
    \item \textbf{World state (LevelDB/CouchDB).} 
    ...
    \item \textbf{Ledger history (blockchain log).} 
