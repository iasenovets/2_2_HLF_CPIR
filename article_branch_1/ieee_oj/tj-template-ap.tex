...
All experiments were conducted on a dedicated host with an Intel~i7 CPU, 32~GB RAM, and Ubuntu~22.04. 
The implementation uses Go~1.22, Lattigo~v6, and Hyperledger Fabric~v2.5 with CouchDB as the world-state database. 
Each Fabric channel (\texttt{channel\_mini}, \texttt{channel\_mid}, \texttt{channel\_rich}) runs an independent CPIR chaincode instance.
Unless otherwise stated, each measurement is averaged over 20 runs with cold and warm world-state cache conditions.
Evaluate transactions are read-only and do not generate blocks; only \textsf{InitLedger} is submitted through the consensus path.

\subsection{Cryptographic Evaluation}
...
\begin{table}[!h]
\caption{BGV Parameter Configuration per Channel}
\label{tab:params}
\centering
    \begin{tabular}{|c|c|c|c|c|c|c|}
    \hline
    Channel & $\log N$ & $N$ & $\log Q_i$ & $\log P_i$ & $T$ & $record_s$ \\
    \hline
    Mini & 13 & 8192 & [60,60,60,38] & [60] & 65537 & 128 \\
    Mid & 14 & 16384 & [60,60,60,38] & [60] & 65537 & 224 \\
    Rich & 15 & 32768 & [60,60,60,38] & [60] & 65537 & 256 \\
    \hline
    \end{tabular}
\end{table}

\textbf{a) End-to-End Latency by Stage.}
Figure~\ref{fig:latency-stages} shows the latency breakdown per cryptographic stage:
\emph{KeyGen} (parameter generation and keypair creation), 
\emph{Enc} (selector encoding and encryption), 
\emph{Eval} (ciphertext–plaintext multiplication), 
and \emph{Dec} (decryption and record reconstruction). 
Each bar represents the average latency for one query at a given $\log N$.

\begin{figure}[!h]
    \centering
    \includegraphics[width=\columnwidth]{figures/plots/e2e_latency_stacked.pdf}
    \caption{End-to-end single-query latency by stage; ct×pt path.}
    \label{fig:latency-stages}
\end{figure}

\textbf{b) Artifact Size Analysis.}
Figure~\ref{fig:artifact-sizes} reports the serialized size (in kb) of cryptographic artifacts: 
public/secret keys, encrypted selector $ct_q$, encrypted response $ct_r$, 
database polynomial $m_{\mathrm{DB}}$, and metadata JSON. 
Base64 encoding adds roughly $33\%$ wire overhead.
...
\textbf{c) Utilization.}
Figure~\ref{fig:scaling} shows end-to-end latency as a function of $record_s$ 
for $\log N \in \{13,14,15\}$. 
Utilization $u=(n \cdot record_s)/N$ quantifies slot occupancy, 
highlighting feasible operating regions under the constraints of Fig.~\ref{fig:exp-matrix}.

\begin{figure}[!h]
\centering
\includegraphics[width=\columnwidth]{figures/plots/scaling_util_utilization_stacked.pdf}
\caption{Scaling of query latency with slot window $record_s$ and utilization $u=(n\cdot record_s)/N$.}
\label{fig:scaling}
\end{figure}
