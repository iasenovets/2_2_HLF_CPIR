...
$ct_r = \emph{Eval}(ct_q, D) = \emph{Enc}_{pk}(d_i)$, returning $ct_r$ to the client for decryption $d_i = \emph{Dec}_{sk}(ct_r)$ 
...
and its corresponding one-hot selector $\hat{v}_i$, the PIR scheme satisfies correctness if:
\[
...
\begin{enumerate}
    ...
    $\mathcal{DR}$ runs \texttt{DecryptResult} (Alg.~S5) to recover $m'(X)$, 
...
\emph{Proof.}
Under the BGV scheme~\cite{cryptoeprint:2011/277}, the encrypted query $ct_q = \emph{Enccoro}_{pk}(m_q)$ is a noisy ciphertext of the form
$ct_q = (c_0, c_1) = (pk_0 \cdot u + T \cdot e_0 + m_q, pk_1 \cdot u + T \cdot e_1) \in R_Q \times R_Q$
that stores the selection mask polynomial $m_q \in R_T$ (encoding the selection vector $v_i \in \{0,1\}^N$). Here,
$u \leftarrow R_Q$ is uniformly random, $e_{pk},e_0,e_1 \leftarrow \mathcal{X}$ are small noise polynomials, and $pk = (pk_0, pk_1) = 
(a \cdot sk + T \cdot e_{pk}, -a)$ is the public key, where $a \leftarrow R_Q$ is uniformly random and $sk \leftarrow \mathcal{X}$ is the 
secret key. Since $m_q$ is hidden by the random term $u$ and the scaled noise $T \cdot e_0$, $ct_q$ is computationally indistinguishable from a
uniformly random element of $R_Q \times R_Q$ to any adversary $\mathcal{A}$ running in time $poly(\lambda)$ without the secret key $sk$. Thus,
$ct_q$ reveals no information about the underlying plaintext coefficients or their positions in $m_q$.
...
The adversary $\mathcal{A}$ selects two indices $i,j \in [n]$, corresponding to mask polynomials $m_{q,i}$ and $m_{q,j}$ 
(encoding $v_i$ and $v_j$ respectively) with coefficients set to $1$ on slots $J_i$ or $J_j$ and $0$ elsewhere.
...
using only $pp$ and $m_{DB}$, the distribution of $View_{\mathcal{A}}$ depends on $b$ only through the underlying $m_{q,b}$.
...
under the $(N,Q,\mathcal{X},T)$-RLWE assumption for any $\mathcal{A}$ running in time $poly(\lambda)$ to be:
$\left| \Pr[b' = b] - \tfrac{1}{2} \right| \le negl(\lambda)$.

\emph{Corollary 1 (Single Read Privacy).} 
For a single query, an adversary $\mathcal{A}$ cannot recover the index $i$ or the data $d_i$, as the view reveals no information about the 
position of the bit in the selection vector $v_i$ beyond what is permitted in $\mathcal{L}$, reducing the probability of a successful 
guess to $\Pr[\mathcal{A}(View_{\mathcal{A}}) \in \{i, d_i\}] \le negl(\lambda)$.

\emph{Corollary 2 (Repeated Reads).}
(1) Following the proof above, any $\mathcal{A}$ views each $ct_q^{(t)} \approx_c ct_q^{(t')}$,
...
a uniform ciphertext--plaintext multiplication of $ct_q^{(t)} = \emph{Enc}_{pk}(m_q^{(t)})=\emph{enc}^{poly}({v_{i}^{(t)}})$ with 
$m_{DB}$ over $R_Q$. This operation processes every coefficient of $m_{DB}$ regardless of the mask $m_q^{(t)}$ and its $J_{i}^{(t)}$,
revealing nothing about $i$ beyond $\mathcal{L}$ for any $\mathcal{A}$ running in time $poly(\lambda)$.
