...
corresponds to the desired record at index $i$. He then encrypts it as $ct_q = \emph{Enc}_{pk}(\hat{v}_i)$ and sends $ct_q$ to the server,
which computes $ct_r = \emph{Eval}(ct_q, D) = \emph{Enc}(d_i)$, returning $ct_r$ to the clientfor decryption $d_i = \emph{Dec}_{sk}(ct_r)$
...
We instantiate Computational PIR as a tuple of algorithms (\emph{KeyGen}, 
...
\begin{itemize}
    ...
    \item $\emph{Enc}_{pk}(\hat{v}_i) \rightarrow ct_q$: Given a record-level one-hot selector $\hat{v}_i$, expand it into $v_i \in \{0,1\}^N$, then
    encode it into the plaintext ring $R_T$, and encrypt it as $ct_q$ under $pk$.
    ...
    \item $\emph{Dec}_{sk}(ct_r) \rightarrow d_i$: Using secret key $sk$, decrypt and decode $ct_r$, then extract window $J_i$
    to recover the desired record $d_i$.
\end{itemize}

\noindent \textbf{Correctness.} 
Given a database $D=\{d_0,\dots,d_{n-1}\}$ stored as a plaintext polynomial \emph{$m_{DB}$} in $R_T$, for any index $i \in [n]$
and its corresponding one-hot selector $\hat{v}_i$, with $\emph{Enc}$ and $\emph{Dec}$ understood as above, the PIR scheme satisfies correctness if:
\[
...
\renewcommand{\arraystretch}{1.1}
    ...
    \hline
    \emph{$m_{DB}$} & Encoded plaintext polynomial \\ & representation of $D$ \\
    \hline
    $\hat{v}_i$ & Record-level one-hot selector for index $i$ \\
    \hline
    ${v}_i$ & Windowed logical-slot selector for index $i$ \\ & with $record_s$ contiguous ones \\
    \hline
    ${c}$ & Logical-slot vector $(c_0,\dots,c_{N-1})$ \\
    \hline
    $J_i$ & Disjoint logical-slot window for index $i$ \\
    \hline
...
\subsection{SECURITY ASSUMPTIONS AND THREAT MODEL}\label{sec:threat_model}
...
\end{equation}
where $bytesPerSlot = \frac{\log_2(T)}{8}$ given plaintext modulus $T$. With $T=256$, $\log_2(256) = 8$
...
in the logical-slot vector $c = (c_0, c_1, \dots, c_{N-1})$:
\begin{equation}
\label{eq:packing}
...
and $J_i[k] = i \cdot record_s + k$ is the $k$-th slot index in window $J_i$.
Each $c_j \in [0,T-1]$ is the $j$-th coordinate of the logical-slot vector $c$.
...
\textbf{Step 4: Encode into Plaintext Polynomial.}
Finally, BGV batching encoder maps vector $c$ to a plaintext polynomial:
\begin{equation}
\label{eq:polynomial_db}
\emph{$m_{DB}$}(X) = \sum_{j=0}^{N-1}\widetilde{c}_j X^j \in R_T,
\end{equation}
where $R_T = \mathbb{Z}_T[X]/(X^N+1)$. Here, $\widetilde{c}$ denotes the polynomial coefficients produced when BGV encodes
the logical-slot vector $c$; generally, $\widetilde{c}\neq c$. Encoding the query with the same encoder makes $\emph{Eval}$ decode
slotwise as $y_j=(v_i)_j c_j$.
This polynomial serves as the database representation \emph{$m_{DB}$} in the PIR protocol, and the client extracts the logical slots
corresponding to the requested record.
...
\begin{equation}
    ...
    \mathcal{S} = \{64, 128, 224, 256, 384, 512\} \ \text{slots}.
\end{equation}
...
\begin{enumerate}
    ...
    zeros pad unused slots, and $c$ is encoded into the plaintext polynomial $\emph{$m_{DB}$}(X)$ in~\eqref{eq:polynomial_db} at the maximum level.
    ...
    For index $i\in[n]$, $\mathcal{DR}$ runs \texttt{FormSelectionVector} (Alg.~S3): 
    define $J_i$, set ${v}_i$ with ones on $J_i$, encode it as $m_q\in R_T$, encrypt as $ct_q=\emph{Enc}_{pk}(m_q)$,
    and serialize/Base64-encode. 

    \item \textbf{PIR query evaluation.}  
    $\mathcal{DR}$ issues \texttt{PIRQuery}($ct_q^{B64}$) (Alg.~S4) via \emph{evaluate}. 
    ...
    $\mathcal{DR}$ runs \texttt{DecryptResult} (Alg.~S5) to recover response plaintext $m'(X)$, decode it into $c$,
...
Extending to multi-record queries is feasible by modifying the selection vector to include multiple $J_i$.

\section{SECURITY ANALYSIS}\label{sec:security}
We now demonstrate how our construction satisfies the security objective defined in Section~\ref{sec:proposed_system}-\ref{sec:threat_model}
...
\emph{Proof.}
Under the BGV scheme~\cite{cryptoeprint:2011/277}, the encrypted query $ct_q = \emph{Enc}_{pk}(m_q)$ is a noisy ciphertext of the form
$ct_q = (c_0, c_1) = (pk_0 \cdot u + T \cdot e_0 + m_q, pk_1 \cdot u + T \cdot e_1) \in R_Q \times R_Q$
that stores the encoded selection mask $m_q \in R_T$ for logical-slot selector $v_i \in \{0,1\}^N$. Here,
$u\leftarrow\mathcal{X}_s$, $e_{pk},e_0,e_1\leftarrow\mathcal{X}_e$ are small polynomials drawn from
the secret and error distributions, respectively, and $pk = (pk_0, pk_1) = (a \cdot sk + T \cdot e_{pk}, -a)$ is the public key,
where $a \leftarrow R_Q$ is uniformly random and $sk \leftarrow \mathcal{X}_s$ is the secret key. Since $m_q$ is hidden by the fresh
terms $(pk_0\cdot u+T\cdot e_0)$ and $(pk_1\cdot u+T\cdot e_1)$, under RLWE we have $ct_q \approx_c U$ for any adversary $\mathcal{A}$ running in
time $poly(\lambda)$ without the secret key $sk$, where $U\leftarrow R_Q \times R_Q$ is a uniformly random element.
Thus, $ct_q$ reveals no information about $m_q$ or its selected logical-slot window $J_i$.

\emph{Indistinguishability Game.}
Consider an adversary $\mathcal{A}$ running in time $poly(\lambda)$ and controlling an honest-but-curious endorsing peer $\mathcal{DO}$.
The adversary $\mathcal{A}$ selects two indices $i,j \in [n]$, corresponding to mask polynomials $m_{q,i}$ and $m_{q,j}$ obtained by
encoding $v_i$ and $v_j$, whose logical slots are set to $1$ on $J_i$ or $J_j$ and $0$ elsewhere.
A challenger $\mathcal{C}$ samples a random bit $b \leftarrow \{0,1\}$ and sets
$m_{q,b} = m_{q,i}$ if $b=0$, or $m_{q,b} = m_{q,j}$ if $b=1$. $\mathcal{C}$ then encrypts the query
$ct_q \gets \emph{Enc}_{pk}(m_{q,b})$ and evaluates the protocol to obtain $ct_r = \emph{Eval}(ct_q, m_{DB})$. 
After this, $\mathcal{C}$ provides the view $View_{\mathcal{A}} = \{ct_q, ct_r, m_{DB}, pp, \mathcal{L}\}$ to $\mathcal{A}$. 
Finally, $\mathcal{A}$ outputs a bit $b'$ and wins if $b' = b$.
Since $ct_q$ is obtained through IND-CPA secure $\emph{Enc}_{pk}(\cdot)$ and $ct_r$ is the result of a deterministic $\emph{Eval}(\cdot)$ 
using $ct_q$, $pp$ and $m_{DB}$, the distribution of $View_{\mathcal{A}}$ depends on $b$ only through the underlying $m_{q,b}$.
Hence, any $\mathcal{A}$ running in time $poly(\lambda)$ capable of distinguishing $View_{\mathcal{A}}^{(i)}$ from 
$View_{\mathcal{A}}^{(j)}$ with non-negligible advantage can be reduced to an $\mathcal{A'}$ that breaks the IND-CPA security of the BGV scheme 
with the same advantage. This contradicts the RLWE assumption~\cite{peikert_lattice_2014}. Thus, we conclude\footnote{See App.~6 
in~\cite{mahdavi_inspire_2025} and Cor.~2 in~\cite{simplepir_2023} for similar \emph{formal} proofs.} query indistinguishability 
under the $(N,Q,\mathcal{X}_{s,e},T)$-RLWE assumption for any $\mathcal{A}$ running in time $poly(\lambda)$ to be:
$\Pr[b' = b] \leq \tfrac{1}{2} + negl(\lambda)$.

\emph{Corollary 1 (Single Read Privacy).} 
For a single query with uniformly sampled $i\in[n]$, an adversary $\mathcal{A}$ cannot recover $i$ or determine which $d_i$ is requested, since the view reveals
no information about the position of the bit ($J_i$) in $v_i$ beyond what is permitted in $\mathcal{L}$, and hence its guessing probability satisfies
$\Pr[\mathcal{A}(View_{\mathcal{A}}) \in \{(i,d_i)\}] \leq \tfrac{1}{n} + negl(\lambda)$.

\emph{Corollary 2 (Repeated Reads).}
(1) Following the proof above, any $\mathcal{A}$ views each $ct_{q,i}^{(t)} \approx_c ct_{q,j}^{(t')}$,
for any $i,j \in [n]$ and any round $t,t' \in \mathbb{N}$ (even if $i = j$ and/or $t \neq t'$).
By a standard hybrid argument, any $\mathcal{A}$ running in time $poly(\lambda)$ sees
$View_{\mathcal{A}}(\mathcal{Q}) \approx_c View_{\mathcal{A}}(\mathcal{Q}')$ for any two query sequences $\mathcal{Q}, \mathcal{Q}'$
of equal length.
(2) Each evaluation $ct_r^{(t)} = \emph{Eval}(ct_q^{(t)}, m_{DB})$ involves
a ciphertext--plaintext multiplication of $ct_q^{(t)}=\emph{Enc}_{pk}(m_q^{(t)})$
with $m_{DB}$ over $R_Q$. This operation is independent of the mask $m_q^{(t)}$ and its corresponding $J_i^{(t)}$;
by (1), the resulting view reveals nothing about $i$ beyond $\mathcal{L}$ to any $\mathcal{A}$ running in time $poly(\lambda)$.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{arturiasenovets.png}}]{ARTUR IASENOVETS}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{feitang.png}}]{FEI TANG}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{huihuizhu.jpg}}]{HUIHUI ZHU}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{pingwang.jpg}}]{PING WANG}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{leiliu.jpg}}]{LEI LIU}
