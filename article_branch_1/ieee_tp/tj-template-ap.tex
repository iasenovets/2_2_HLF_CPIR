...
\begin{abstract}
    ...
    This paper proposes a Private Information Retrieval (PIR) mechanism to enable \emph{private reads} from Hyperledger Fabric’s world state, 
    allowing endorsing peers to process encrypted queries without learning which record is accessed. We implement and benchmark a PIR-enabled 
    ...
    near-linear growth: block size increases from 77~KB to 294~KB and world-state from 112~KB to 332~KB as the ring dimension 
    scales from $2^{13}$ to $2^{15}$. Parameter analysis further indicates that ring size and record length jointly constrain 
    packing capacity, supporting up to 512~records of 64~bytes each under $2^{15}$. These results confirm the practicality of 
    PIR-based private reads in permissioned ledgers and highlight future directions toward sublinear, constant-time, and verifiable 
    query execution.
\end{abstract}
...
``PSE Roadmap: 2025 and Beyond''~\cite{ethereum_pse_nodate}, where they also 
...
However, integrating PIR directly into Fabric’s chaincode layer introduces practical challenges related to performance, 
communication overhead, and parameter tuning.
...
\textbf{Information-Theoretic.} IT-PIR schemes~\cite{chor_private_1998,demmler_raid-pir_2014,goldberg_improving_2007,
beimel_breaking_2002, devet_optimally_2012} provide unconditional privacy by distributing the database across \textbf{multiple 
...
Older~\cite{cachin_computationally_1999,gentry_single-database_2005, 
brakerski_efficient_2011, dong_fast_2014, aguilar-melchor_xpir_2016} and more recent non-preprocessing single-server 
schemes such as SealPIR~\cite{sealpir_2018}, FastPIR~\cite{fastpir_2021}, OnionPIR~\cite{onionpir_2021}, and Spiral~\cite{spiral_2022} rely on 
cryptographic assumptions, such as homomorphic encryption (HE) to achieve privacy using \textbf{single untrusted server}. 
...
Beimel et al.~\cite{beimel_reducing_2004} introduced the concept of PIR with preprocessing, where a preprocessing (offline) phase is 
performed before the actual query (online) phase to reduce online communication and computation costs.
...
Moreover, these schemes introduce significant storage blowup: for instance, SimplePIR’s~\cite{simplepir_2023} precomputation 
tables or HintlessPIR’s~\cite{hintless_pir_2024} server hints can exceed several times the original database size,
and Piano's~\cite{piano_pir_2024} client preprocessing requires full-database downloads in its initialization stage.  
Such assumptions are incompatible with blockchain environments, where deterministic, replayable, and stateless transaction execution is critical.
...
HE-based PIR is the most viable option for enabling \emph{private reads} in Fabric-like permissioned ledgers, for the following reasons:
(i) it requires no changes to Fabric’s consensus, core architecture or endorsement policies,
(ii) it avoids the need for multiple non-colluding servers or endorsing peers in our case,
(iii) it maintains Fabric’s stateless transaction model without per-client state on peers,
(iv) it avoids offline preprocessing phases incompatible with Fabric’s on-demand \emph{evaluate} calls,
(v) it avoids offline communication or storage blowup on clients or peers,
(vi) it supports single-round query-response interactions naturally compatible with Fabric's \emph{evaluate} calls,
(vii) it leverages existing homomorphic encryption libraries that can be integrated into chaincode, and
(viii) it achieves somewhat practical performance under certain parameter choices, as we demonstrate in this work.
...
\textbf{Targeted gap.}
Blockchains like Hyperledger Fabric ensure integrity and auditability of shared data but leak function arguments 
and read-sets to endorsing peers during both \emph{evaluate} and \emph{submit} calls, creating privacy risks in multi‑organisation settings. 
While prior works have explored PIR integration in blockchain contexts, none have specifically addressed
the challenge of enabling \emph{private reads} directly within Fabric’s chaincode layer.
To fill this gap, we explore how to enable \emph{private reads} from Fabric’s world state by integrating a HE-based PIR mechanism directly 
into chaincode, and we evaluate its practicality through detailed benchmarks and parameter studies.
...
Hyperledger Fabric under \emph{evaluate} calls and achieve somewhat practical performance under certain parametrization choices.

Hence, the main contributions of this work are:
\begin{enumerate}
    \item \textbf{First PIR-on-Blockchain system for private reads.}
    We present the first end-to-end design, implementation, and evaluation of a HE-based PIR mechanism integrated directly into
    Hyperledger Fabric chaincode, enabling private reads from world state without modifying Fabric’s core architecture.
    \item \textbf{Polynomial database construction.}
    We derive necessary conditions on the cryptographic parameters, database size, and record structure to encode domain-specific 
    structured key-value records into plaintext-polynomial format suitable for HE-based PIR, ensuring retrieval correctness and cryptographic feasibility.
    ...
    where each channel hosts a distinct parameter set and a record template, allowing clients to select the appropriate channel based on 
    their query needs.
...
\begin{itemize}
    ...
    runs $\emph{KeyGen}(\lambda)\!\to\!(pk,sk)$, forms $ct_q=\emph{Enc}_{pk}(v_i)$, calls \emph{evaluate} \emph{PIRQuery}, and later decrypts $ct_r$.
    \item \textbf{Gateway ($\mathcal{GW}$):} The Fabric client/chaincode interface used by $\mathcal{DW}$ 
    and $\mathcal{DR}$ to invoke \emph{InitLedger}, \emph{GetMetadata}, and \emph{PIRQuery}. It follows standard Fabric semantics; no extra trust is assumed.
\end{itemize}
...
\begin{enumerate}
    ...
    $\mathcal{DW}$ invokes \emph{InitLedger} via $\mathcal{GW}$ using \emph{submit}. Chaincode derives $record_s$ from 
    ...
    $\mathcal{DR}$ calls \emph{GetMetadata} via \emph{evaluate} to obtain $n$, $record_s$, and BGV parameters needed
    to form a valid query.
    \item \textbf{Private retrieval.}
    $\mathcal{DR}$ constructs $ct_q=\emph{Enc}_{pk}(v_i)$ and invokes \emph{PIRQuery} via \emph{evaluate}. $\mathcal{DO}$ 
...
In our motivating use case of Cyber Threat Intelligence (CTI) sharing, JSON objects containing fields such as hash 
digests and threat levels are flattened into byte sequences. 
...
For example, if the largest record is $126$ bytes, then $record_s = \lceil 126/1 \rceil = 126$, which rounds up to $128$ 
slots due to the discrete window policy.  
...
if given richer templates. Feasible configurations occur only where all 3 predicates are met, guiding practical parameter selection.
\begin{figure}[!h]
    \centering
...
\begin{algorithm}[!h]
    \caption{\emph{InitLedger} (chaincode)}
...
\begin{algorithm}[!h]
    \caption{\emph{GetMetadata} (chaincode)}
...
\begin{algorithm}[!h]
    \caption{\emph{FormSelectionVector} (client)}
...
\begin{algorithm}[!h]
    \caption{\emph{PIRQuery} (chaincode)}
    \label{alg:pirquery}
    \begin{algorithmic}[1]
    \REQUIRE $ct_q^{B64}$
...
\begin{algorithm}[!h]
    \caption{\emph{DecryptResult} (client)}
...
Client–peer communication was performed through the Fabric~Go~SDK~\cite{fabric_gateway_pkg_client_nodate}. 
...
average server-side execution time of the main chaincode functions across different ring sizes.
...
\begin{table}[!h]
    ...
    \hline
    $N$ & \emph{InitLedger} & \emph{GetMetadata} & \emph{PIRQuery} \\
    \hline
...
network I/O per \emph{PIRQuery} transaction across the three channel configurations.
...
Figure \ref{fig:query-privacy} illustrates the peer- and client-side execution during a PIR query.
...
For every execution of \emph{PIRQuery}, the benchmarking script recorded cumulative network statistics (\emph{NET~I/O}) before and after the transaction, 
and the Python post-processing pipeline computed per-epoch deltas (\emph{bytes\textsubscript{in/out}(t)}~--~\emph{bytes\textsubscript{in/out}(t--1)}) 
...
Table~\ref{tab:comparison-related} contrasts representative blockchain privacy frameworks mentioned in Section~\ref{sec:introduction},
...
\emph{(iv)} the transaction phase in which privacy logic executes (\emph{evaluate} vs. \emph{submit}),
